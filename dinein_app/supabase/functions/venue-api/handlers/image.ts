...
export async function handleUploadVenueImage(
  ...
  if (uploadError) {
    console.error("[venue-api] venue image upload failed", uploadError);
  ...
  if (dbError) {
    console.error("[venue-api] venue image DB update failed", dbError);
    throw new HttpError(
      500,
...
export async function handleUploadMenuItemImage(
  ...
  if (uploadError) {
    console.error("[venue-api] menu item image upload failed", uploadError);
  ...
  if (dbError) {
    console.error("[venue-api] menu item image DB update failed", dbError);
    throw new HttpError(
      500,
...
export async function handleIngestMenuDocument(
  ...
  for (const model of models) {
    try {
      console.log(
        `[venue-api] menu ingest: trying model ${model} for venue ${venueId}`,
      );
      ...
      if (!response.ok) {
        console.warn(
          `[venue-api] menu ingest: model ${model} returned ${response.status}`,
        );
        continue;
      }
      ...
      if (!text) {
        console.warn(
          `[venue-api] menu ingest: model ${model} returned empty text`,
        );
        continue;
      }
      ...
      console.error(`[venue-api] menu ingest: model ${model} error`, error);
      continue;
    }
  }
  ...
  if (insertError) {
    console.error("[venue-api] menu ingest bulk insert failed", insertError);
    throw new HttpError(500, "Could not create the extracted menu items.");
  }

  console.log(
    `[venue-api] menu ingest: created ${
