...
async function bellRequestVenueId(
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) {
    console.error("[venue-api] bell request venue lookup failed", error);
...
export async function handleSendWave(
  ...
  const { data: venueCheck, error: venueCheckError } = await supabase
    ...
    .maybeSingle();
  if (venueCheckError) {
    console.error("[venue-api] wave venue check failed", venueCheckError);
  ...
  const { data: existing, error: existingError } = await supabase
    ...
    .maybeSingle();
  if (existingError) {
    console.error("[venue-api] wave dedupe lookup failed", existingError);
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) {
    console.error("[venue-api] wave insert failed", error);
  ...
  try {
    ...
    console.error("[venue-api] bell request push dispatch failed", error);
  }
...
export async function handleGetBellRequests(
  ...
  if (error) {
    console.error("[venue-api] get bell requests failed", error);
...
export async function handleResolveBellRequest(
  ...
  const { error } = await supabase
    ...
    .eq("id", requestId);
  if (error) {
    console.error("[venue-api] resolve bell request failed", error);
