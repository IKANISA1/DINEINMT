...
export async function handleGetMenuItems(
  ...
  if (venueError) {
    console.error(
      "[menu-api] get menu items venue lookup failed",
      venueError,
    );
  ...
  if (error) {
    console.error("[menu-api] get menu items failed", error);
...
export async function handleGetMenuItemById(
  ...
  if (error) {
    console.error("[menu-api] get menu item by id failed", error);
  ...
  if (venueError) {
    console.error(
      "[menu-api] get menu item by id venue lookup failed",
      venueError,
    );
    throw new HttpError(500, "Could not load the venue.");
  }
...
export async function handleToggleMenuItemAvailability(
  ...
  if (error) {
    console.error("[menu-api] toggle menu item availability failed", error);
...
export async function handleCreateMenuItem(
  ...
  if (error) {
    console.error("[menu-api] create menu item failed", error);
...
export async function handleUpdateMenuItem(
  ...
  if (error) {
    console.error("[menu-api] update menu item failed", error);
...
export async function handleDeleteMenuItem(
  ...
  if (error) {
    console.error("[menu-api] delete menu item failed", error);
...
export async function handleSetMenuItemHighlights(
  ...
  if (itemIds.length > 0) {
    ...
    if (validateError) {
      console.error(
        "[menu-api] validate menu item highlights failed",
        validateError,
      );
  ...
  if (clearError) {
    console.error("[menu-api] clear menu item highlights failed", clearError);
  ...
  for (const [index, itemId] of itemIds.entries()) {
    ...
    if (updateError) {
      console.error(
        "[menu-api] set menu item highlight failed",
        updateError,
      );
  ...
  if (error) {
    console.error("[menu-api] reload menu item highlights failed", error);
