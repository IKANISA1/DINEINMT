...
export async function handleUploadVenueImage(
  ...
  if (uploadError) {
    console.error("[admin-api] venue image upload failed", uploadError);
  ...
  if (dbError) {
    console.error("[admin-api] venue image DB update failed", dbError);
    throw new HttpError(
      500,
...
export async function handleUploadMenuItemImage(
  ...
  if (uploadError) {
    console.error("[admin-api] menu item image upload failed", uploadError);
  ...
  if (dbError) {
    console.error("[admin-api] menu item image DB update failed", dbError);
    throw new HttpError(
      500,
...
export async function handleIngestMenuDocument(
  ...
  for (const model of models) {
    try {
      console.log(
        `[admin-api] menu ingest: trying model ${model} for venue ${venueId}`,
      );
      ...
      if (!response.ok) {
        console.warn(
          `[admin-api] menu ingest: model ${model} returned ${response.status}`,
        );
        continue;
      }
      ...
      if (!text) {
        console.warn(
          `[admin-api] menu ingest: model ${model} returned empty text`,
        );
        continue;
      }
      ...
      console.error(`[admin-api] menu ingest: model ${model} error`, error);
      continue;
    }
  }
  ...
  if (insertError) {
    console.error("[admin-api] menu ingest bulk insert failed", insertError);
    throw new HttpError(500, "Could not create the extracted menu items.");
  }

  console.log(
    `[admin-api] menu ingest: created ${
